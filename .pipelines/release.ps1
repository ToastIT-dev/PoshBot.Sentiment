...
if ($Publish.IsPresent) {
    ...
    Try {
        $Splat = @{
            Path        = (Resolve-Path -Path $env:ArtifactDir\PoshBot.Sentiment)
            NuGetApiKey = $env:PSGallery
            ErrorAction = 'Stop'
        }
        Publish-Module @Splat

        Write-Output -InputObject ('PoshBot.Sentiment PowerShell Module published to the PowerShell Gallery')
    } Catch {
        throw $_
    }
}
